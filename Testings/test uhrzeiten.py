...
if "a" in text.lower():
    if "a" in text:
        converted = converted + text.replace("a", "l72dhlo_", letters)
    if "A" in text:
        converted = converted + text.replace("A", "L72dhmi_", letters)
    print("~~a~~ converted")

if "b" in text.lower():
    if "b" in text:
        converted = converted + text.replace("b", "l78aiko_", letters)
    if "B" in text:
        converted = converted + text.replace("B", "L78aijg_", letters)
    print("~~b~~ converted")

if "c" in text.lower():
    if "c" in text:
        converted = converted + text.replace("c", "l67gsht_", letters)
    if "C" in text:
        converted = converted + text.replace("C", "L11kanm_", letters)
    print("~~c~~ converted")
